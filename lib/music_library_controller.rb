...
class MusicLibraryController
  ...
  def list_songs
    ...
    Song.all.sort! do |song_a, song_b| #returns an array of songs sorted by song name
      song_a.name <=> song_b.name #sorts them alphabetized by name
    end.each.with_index(1) do |song,index|
      puts "#{index}. #{song.name}"  #iterating over the song.all that's been sorted
    end
  end
